...
function pollAgentSignals(sessionId: string): AgentSignal[] {
  ...
  for (const { suffix, side } of SIGNAL_SUFFIXES) {
    ...
    let parsed: { phase?: unknown; summary?: unknown; comment?: unknown; verdict?: unknown; status?: unknown; findings?: unknown; tasks?: unknown } | null = null;
    if (jsonMatch) {
      try {
        parsed = JSON.parse(jsonMatch[0]) as { phase?: unknown; summary?: unknown; comment?: unknown; verdict?: unknown; status?: unknown; findings?: unknown; tasks?: unknown };
      } catch {
        parsed = null;
      }
    }
    ...
    const summary = typeof parsed.summary === "string" ? compactSignalText(parsed.summary, 1200) : undefined;
    const comment = typeof parsed.comment === "string" ? compactSignalText(parsed.comment, 4000) : undefined;
    ...
    if (shouldDedupe) {
      const signalHash = signalPayloadHash({ phase, summary, comment, verdict, status, findings, tasks });
    ...
    signals.push({ side, phase, summary, comment, verdict, status, findings, tasks });
    db.addWorkflowEvent(sessionId, `signal.consumed.${suffix}`, "agent", "ok", undefined, verdict ? `${phase} | ${verdict}` : phase, false);
    recordSignalEvidence(sessionId, side, phase, verdict, findings, tasks, summary);
  }
  return signals;
}
...
function recordSignalEvidence(
  ...
  if (phase === "ready" || phase === "analysis" || phase === "implementation") {
    ...
    db.recordWorkflowEvidence(
      sessionId,
      "deploy",
      `${agentLabel} merged release PRs`,
      summary || `${agentLabel} signaled that release PR merge work completed. Evidence details are in the agent transcript and board comment.`,
      { source: "agent", agentSide: side }
    );
