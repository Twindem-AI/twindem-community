...
export function renderAgentContextBrief(pack: AgentContextPack): string {
  ...
  const agentSignalSuffix = pack.side === "L" ? "A1" : "A2";
  const boardCommentSignal =
    `If the human asks you in chat to write or post a Jira/board comment, do NOT use Jira, a browser, or a Jira CLI. Draft the comment and write \`.twindem/signals/${pack.sessionId}.${agentSignalSuffix}.json\` containing exactly {"phase":"comment","comment":"<comment body>"}; Twindem will post it through the configured Board service layer.`;
