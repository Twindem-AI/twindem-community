...
function App() {
  ...
  async function handleAgentSignal(signal: AgentSignal, polledSessionId: string) {
    ...
    if (signal.phase === "deployed" && signal.side === "R") {
      await window.tandem.evidence
        ...
        .catch(() => undefined);
      setNotice("Agent 2 confirmed the UAT deploy ✓");
      return;
    }
  ...
  async function mergeTaskPRs() {
    const current = detailRef.current ?? detail;
    if (!current?.session.repo || !current.session.issueNumber) {
      setErrorNotice("There's no board task with PRs to merge yet.");
      return;
    }
    await run(async () => {
      await submitInstruction("L", mergeInstruction(detailRef.current ?? current, agentSignature("L")));
      setNotice("Asked Agent 1 to merge the task's PRs — approve the gh merge in its terminal.");
    });
  }
            ...
            <div className="phase-stepper">
              ...
              {activeIdeaType.requiresImplementation && (
                <button
                  ...
                  disabled={!phaseHasIssue || phaseReached !== 2 || Boolean(phaseActionPending)}
                  title="Merge the task's open PR(s) — delegates to Agent 1 (it knows the branches/PRs). Available during Implement, after the code review passes and before UAT."
...
function implementInstruction(
  ...
  if (!type.requiresImplementation) {
    ...
    return [
      ...
      "When done, post ONE COMMENT on the issue summarizing the artifact produced, where it lives, evidence gathered, validation performed, open risks, and the recommended next action.",
      signCommentsLine(signature),
      `Then signal that you are ready for review: ${signalLine(detail.session.id, "A1", '{"phase":"ready"}')}`,
      NO_CHAT_MARKERS
    ].join("\n");
  }
  return [
    ...
    "When done, post ONE COMMENT on the issue summarizing what you implemented: the branch name, PR link if one was opened, files/areas changed, and tests run. Do NOT give a verdict and do NOT re-post the plan.",
    signCommentsLine(signature),
    `Then signal that you are ready for review: ${signalLine(detail.session.id, "A1", '{"phase":"ready"}')}`,
    NO_CHAT_MARKERS
  ].join("\n");
...
function releaseOpsInstruction(detail: SessionDetail, target: "UAT" | "PRODUCTION", runbook: string, signature?: string): string {
  ...
  return [
    ...
    runbook,
    "----- END OF INSTRUCTIONS -----",
    "When finished, post a COMMENT on the issue with concrete evidence: commands run, target, result, and any verification performed.",
    signCommentsLine(signature),
    `On SUCCESS, signal completion so Twindem can advance the task: ${signalLine(detail.session.id, "A2", target === "PRODUCTION" ? '{"phase":"released"}' : '{"phase":"deployed"}')}`,
...
function uatFixInstruction(detail: SessionDetail, finding: string, signature?: string): string {
  ...
  return [
    ...
    "Post a COMMENT summarizing the fix (what was wrong, what you changed).",
    signCommentsLine(signature),
    `When the fix is ready for re-review/re-deploy, ${signalLine(detail.session.id, "A1", '{"phase":"ready"}')}`,
    NO_CHAT_MARKERS
  ].join("\n");
}

// A1: merge the task's open PR(s). The actual merge is gated by the CLI permission prompt.
function mergeInstruction(detail: SessionDetail, signature?: string): string {
  ...
  return [
    ...
    ...(url ? [`URL: ${url}`] : []),
    "You are Agent 1 (Implementer). Merge the OPEN pull request(s) you opened for this task.",
    `For EACH affected repo: find the task's PR (e.g. \`gh pr list --head task/${detail.session.issueNumber}-...\`) and merge it (\`gh pr merge <pr> --squash --delete-branch\`, or that repo's convention).`,
    ...
    "After merging, post ONE comment on the issue listing what was merged (repo + PR + merge commit) and anything that could not be merged.",
    signCommentsLine(signature),
    `Then ${signalLine(detail.session.id, "A1", '{"phase":"ready"}')}`,
    NO_CHAT_MARKERS
  ].join("\n");
}
...
function rollbackInstruction(detail: SessionDetail, signature?: string): string {
  ...
  return [
    ...
    "Record exactly what you rolled back (commit/PR + reason) as a COMMENT on the issue.",
    signCommentsLine(signature),
    `When done, ${signalLine(detail.session.id, "A2", '{"phase":"rollback"}')}`,
    NO_CHAT_MARKERS
  ].join("\n");
}
